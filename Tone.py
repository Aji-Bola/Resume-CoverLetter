...
class ResumeTool:
    ...
    def generate_tailored_resume(self) -> str:
        prompt = (
        f"Tailor this resume to the job below using a {self.tone} tone and {self.length} length.\n"
    ...
    def generate_cover_letter(self) -> str:
        prompt = (
        f"Write a {self.length}, {self.tone}-toned cover letter for the job below.\n"
        f"Resume:\n{self.resume_text}\nJob Description:\n{self.job_description}"
    )
...
class ResumeApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.appearance_menu.grid(row=11, column=1, sticky="w")
